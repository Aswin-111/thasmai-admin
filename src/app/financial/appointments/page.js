...
function Appointments() {
	...
	async function handleSearch(page, newRow) {
		...
		try {
			...
			const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/superadmin/ashram-search?field=${searchRef.current.value}&value=${textRef.current.value}&page=${searchPage}&limit=${rows}`);
			console.log(response);
