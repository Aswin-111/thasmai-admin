...
import { useDonationFilterStore } from './filterState'
  ...
  // useEffect(()=>{filterState.setFieldText(filterState.fieldValue)},[filterState.fieldValue])  
	...
	async function handleSearch(page, newRow) {
		...
		try {
			...
			const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/superadmin/donation-search?field=${searchRef.current.value}&value=${textRef.current.value}&page=${searchPage}&limit=${rows}`);
