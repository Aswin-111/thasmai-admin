...
function DeleteContactPopUp(props) {
    ...
    const handleDelete = async () => {
        ...
        try {
            const response = await axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/superadmin/deleteContact/${contactId}`);
            console.log(response);
            toast.success("Contact deleted successfully.");
            props.setIsContactTableRenderToggle(prevValue => !prevValue);
            props.setIsDeleteContactOpen(false);
        ...
        <div className="w-[100vw] h-[100vh] bg-[#000000af] absolute left-0 top-0 flex justify-center items-center">
            <div className='w-[450px] h-[200px] bg-white rounded hover:bg-[#cfcdcc]'>
                <div className='w-full h-[50%] flex justify-center items-center'>
                    <p className='text-lg font-medium text-black'>Are you sure you want to delete this contact?</p>
                </div>
